...
def prob(curr, new):
    ...
    res = math.tanh((new - curr) / 2) * 0.5 + 0.5
...
try:
    progress = glob.glob("C:/SSHS/814_C/*.txt")
    temp = read(progress[-(random.randrange(0, 5) % len(progress)) - 1])
...
for i in range(ITER-1, -1, -1):
    ...
    if random.uniform(0, 1) < 0.05: T = 2
    ...
    if curr_val > MAX:
        ...
        f = open("C:/SSHS/814_C/{:04.0f}.txt".format(MAX), "w+")
